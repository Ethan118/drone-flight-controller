...
void configureChannelMapping() {
    ...
    mode_mapping[PITCH]    = CHANNEL4;
    mode_mapping[ROLL]     = CHANNEL2;
...
bool isStarted() {
    ...
    if (status == STOPPED && pulse_length[mode_mapping[YAW]] <= 1315 && pulse_length[mode_mapping[THROTTLE]] <= 1012) {
        status = STARTING;
    }

    // When left stick is moved back in the center position
    if (status == STARTING && pulse_length[mode_mapping[THROTTLE]] <= 1012) {
        status = STARTED;
...
void calculateSetPoints() {
    ...
    pid_set_points[PITCH] = calculateSetPoint(gyro_angle[PITCH], pulse_length[mode_mapping[PITCH]]);
    pid_set_points[ROLL]  = calculateSetPoint(gyro_angle[ROLL], pulse_length[mode_mapping[ROLL]]);
}
